...
import com.kob.util.HttpClientUtil;
...
public class AcAppServiceImpl implements AcAppService {
    ...
    public Result<?> applyCode() {
        ...
        try {
            ...
            log.error("Failed to encode redirect URI", e);
            return Result.error();
        }
    ...
    public Result<?> receiveCode(AcCodeVO acCodeVO) {
        ...
        // 根据 openid 判断用户是否存在
        List<User> list = userService.list(Wrappers.<User>lambdaQuery().eq(User::getOpenId, openid));
        if (!list.isEmpty()) { // 存在
        ...
        if (username == null || photo == null)
            return Result.error();
        for (int i = 0; i < 100; i++) {
            if (userService.list(Wrappers.<User>lambdaQuery()
                    .eq(User::getUsername, username)).isEmpty())
                break;
